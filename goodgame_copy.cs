...
public class TheWindow : Window
{
    ...
    private TheWindow() // constructor
    {
        ...
        //
        if (!File.Exists(path + pathName)) File.CreateText(path + pathName);
        string loaded = new FileInfo(path + pathName).Length > 0 ? File.ReadLines(path + pathName).First() : ""; // grab the network line
        ...
        if (textBoxStr[0] == loaded) // load and override weights
    ...
    ~TheWindow() { /* dropped? */ }
    private void Window_Closed(object sender, EventArgs e) // destructor alternative
    {
            ...
            for (int i = 0, j = input; i < layer; i++)
            {
                ...
                for (int k = 0, kEnd = u[i + 1], pos = -1; k < kEnd; k++, j++)
                    if (xstTmp < gpx && xstTmp + tile > gpx && yst[j] < gpy && yst[j] + tile > gpy)
                    {
                        ...
                        {
                            clickedNeuron = j;
                            //   TailorMadeGrowing((pos = /*j + 1*/ (j - k) + kEnd), i); // add clicked position + 1
                            CopyNode((pos = /*j + 1*/ (j - k) + kEnd), i, j, 10000, i); // add clicked position + 1
                        }
    ...
    void CopyNode(int pos, int neuronLayer, int tarPos, int x, int iCurrent)
    {
        ...
        for (int i = 0, j = input, t = 0, w = 0, c = 0; i < layer; i++, t += u[i - 1], w += u[i] * u[i - 1])
            for (int k = 0, ke = u[i + 1]; k < ke; k++, j++) for (int n = t, ne = u[i], m = w + k; n < t + ne; n++, m += ke)
                    if (tarPos == j || tarPos == n) // if ingoing or outgoing cons of that neuron
                    {
                        targetDeltas[c] = delta[m];
                        targetWeights[c++] = weight[m];
                    }

        // 3. add this neuron to its layer and resize the network
        u[neuronLayer + 1] += 1; InitLogic();
        ...
        // 5. restore weights or add the new ones
        Erratic rnd = new Erratic(mode == 3 ? pos : FastRand());
        for (int i = 0, w = 0, j = 0, c = 0; i < layer; i++, w += u[i] * u[i - 1])
        {
    ...
    void NeuralNetworkRun(bool training)
    {
        ...
        for (int x = 1; x < len + 1; x++)
        {
            ...
            // backprop and optimizer (rosenblatt) for training 
    ...
    void NeuralNetworkRosenblattOptimizer()
    {
        for (int i = 0, mStep = 0; i < layer; i++, mStep += u[i] * u[i - 1]) // layer
        {
            ...
            for (int m = mStep, mEnd = mStep + u[i] * u[i + 1]; m < mEnd; m++) // weight (don't need the neuron loop)
            {
                ...
                if (s2 > oneUp) continue; // check for overwhelming deltas to keep the change low     
                weight[m] += del * lr;
